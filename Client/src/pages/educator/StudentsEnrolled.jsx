...
const StudentsEnrolled = () => {
  ...
  const fetchEnrolledStudentsData= async ()=>{
    try {
      ...
      const { data } = await axios.get(backendUrl+'/api/educator/enorlled-students',{ headers: { Authorization: `Bearer ${token}` } })
  ...
  return enrolledStudents ? (
    <div className='h-screen flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <div className='flex flex-col items-center max-w-4xl w-full bg-white border border-gray-500/20 overflow-hidden rounded-md'>
        <table className='md:table-auto items-start table-fixed w-full overflow-hidden pb-4'>
          ...
          <tbody className='text-sm text-gray-500'>
            {enrolledStudents.map((item,index)=>(
              <tr key={index} className='border-b border-gray-500/20'>
                ...
                <td className='py-4 px-3 truncate'>{item.courseTitle}</td>
